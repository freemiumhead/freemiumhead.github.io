...
gulp.task('jade:build', function () {
	gulp.src(path.src.jade) //Выберем файлы по нужному пути
		...
		.pipe(jade({pretty: true})) // Прогоняем через компилятор и если нужно добавляем опцию для читаемости кода {pretty: true}
...
gulp.task('js:build', function () {
	gulp.src(path.src.js) //Найдем наш main файл
		.pipe(plumber({errorHandler: notify.onError("Error: <%= error.message %>")}))
		.pipe(rigger()) // Прогоним через риггер
...
gulp.task('style:build', function () {
	gulp.src(path.src.style) //Выберем наш main.scss или main.style
		...
		.pipe(stylus({'include css': true})) // включим импорт css и Скомпилируем 
